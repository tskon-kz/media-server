...
Чтобы включить автодеплой при каждом пуше в `main`, добавь секреты в репозиторий (**Settings → Secrets and variables → Actions**):
...
При каждом пуше: собирается новый Docker-образ → пушится в `ghcr.io` → через Watchtower запускается обновление на сервере. Шаг пропускается, если `SERVER_HOST` не задан.
